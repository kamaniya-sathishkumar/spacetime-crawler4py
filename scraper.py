...
def extract_next_links(url, resp):
    ...
    lst = []
    # pages = html.fromstring(resp.raw_response.content)
    #if type(resp) != None:
...
def is_valid(url):
    ...
    try:
        ...
        #if (not re.match(r"^((?!today.uci.edu).)*$", parsed.netloc.lower()) and re.match(r'^(/department/information_computer_sciences).*$', parsed.path.lower())):
